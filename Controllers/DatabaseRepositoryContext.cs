...
public class DatabaseRepositorycContext
{
    ...
    private string _loggedOrgName = string.Empty;
    private string _loggedRoleName = string.Empty;
    ...
    // public DatabaseRepositorycContext(IConfiguration configuration)
    {
        ...
        _loggedOrgName = orgName;// HttpContext.User.FindFirst("OrganizationName")?.Value;
        _loggedRoleName = rolename;
    }
    ...
    public List<int> GetTestStageDBData()
    {
        ...
        try
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                var query = @" SELECT ";
                if (_loggedRoleName != "CG Superadmin")
                {
                ...
                if (_loggedRoleName != "CG Superadmin")
                {
                    query = query + " WHERE organization  = '" + _loggedOrgName + "'";
                }
                query = query + ") as test, " +
                    "(SELECT SUM(CASE WHEN StartTime >= " + startdashboardfromdate + " AND StartTime < CURRENT_DATE() + INTERVAL 1 DAY THEN 1 ELSE 0 END) " +
                ...
                if (_loggedRoleName != "CG Superadmin")
                {
                    query = query + " AND organization  = '" + _loggedOrgName + "'";
                }
                ...
                if (_loggedRoleName != "CG Superadmin")
                {
                    query2 = " AND organisation = '" + _loggedOrgName + "'" + " GROUP BY organisation";
                }
                query = query + query2;

                using (var command = new MySqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@orgName", _loggedOrgName);
        ...
        try
        {
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                using (var command = new MySqlCommand(spname, connection))
                {
                    ...
                    {
                        command.Parameters.AddWithValue("@orgname", "");
                    }
    ...
    public List<int> GetSumOfTestsDBData(string orgName)
    {
        ...
        using (var connection = new MySqlConnection(_connectionString))
        {
            ...
            if (_loggedRoleName != "CG Superadmin")
            {
                query2 = " AND organisation = '" + _loggedOrgName + "' ";
            }
            query = query + query2;
    ...
    public List<int> GetDaywiseOfValidationDBData()
    {
        ...
        using (var connection = new MySqlConnection(_connectionString))
        {
            ...
            if (_loggedRoleName != "CG Superadmin")
            {
                query2 = " WHERE organisation = '" + _loggedOrgName + "' ";
            }
    ...
    public List<object> GetTestSummaryDBData()
    {
        ...
        try
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                if (_loggedRoleName != "CG Superadmin")
                {
                    query2 = " AND temp.organization = '" + _loggedOrgName + "'";
                }
        ...
        try
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                var query = @"SELECT `CGStagId`,`CandidateId`,`TestCode`,`Starttime`,`Endtime`,`name`,`ReportName`, `teststatus`,`Mobile_No`, `reporturl`, " +
                        ...
                        " FROM `cgstagingdata`  ";
                query = query + " WHERE  Testcode = '" + testcode + "' AND Starttime >= " + startdashboardfromdate + " AND Starttime < CURRENT_DATE() ";
                if (_loggedRoleName != "CG Superadmin")
                {
                    query2 = " AND organization = '" + _loggedOrgName + "'";
                }
                query = query + query2;
    ...
    public List<object> GetTestPerdayMonthwiseDBData()
    {
        ...
        try
        {
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                //                    "  organization  = '" + _loggedOrgName + "') as test, " +
                //                "(SELECT SUM(CASE WHEN StartTime >= DATE(day) AND StartTime < DATE(day) + INTERVAL 1 DAY THEN 1 ELSE 0 END) " +
                //                    " FROM cgstagingdata  WHERE reporturl IS NOT NULL " +
                //                " AND reporturl != '' " +
                //                " AND organization  = '" + _loggedOrgName + "') as completed " +
                //            " FROM validation_table " +
                //            " WHERE " +
                //"timestamp BETWEEN " + startdashboardfromdate + " AND CURDATE() ";
                var query = @"WITH Last30Days AS(
                              SELECT CURDATE() -INTERVAL n DAY AS day
                              FROM(SELECT ROW_NUMBER() OVER() - 1 AS n FROM validation_table LIMIT 30) AS numbers
                            )
                            SELECT
                              d.day,
                              COALESCE(SUM(CASE WHEN DATE(v.timestamp) = d.day AND v.login_page = 1 THEN 1 ELSE 0 END), 0) AS newuser,
                              COALESCE(SUM(CASE WHEN DATE(v.timestamp) = d.day AND v.register_page = 1 THEN 1 ELSE 0 END), 0) AS register,
                              (SELECT SUM(CASE WHEN cg.StartTime >= d.day AND cg.StartTime < d.day + INTERVAL 1 DAY THEN 1 ELSE 0 END)
                               FROM cgstagingdata cg ";
                if (_loggedRoleName != "CG Superadmin")
                {
                    query = query + " WHERE cg.organization '" + _loggedOrgName + "'";
                }
                query = query + ") AS test, " +
                  "(SELECT SUM(CASE WHEN cg.StartTime >= d.day AND cg.StartTime < d.day + INTERVAL 1 DAY THEN 1 ELSE 0 END) " +
                  " FROM cgstagingdata cg " +
                  " WHERE cg.reporturl IS NOT NULL AND cg.reporturl != '' ";
                if (_loggedRoleName != "CG Superadmin")
                {
                    query = query + " AND cg.organization '" + _loggedOrgName + "'";
                }
                query = query + ") AS completed FROM Last30Days d " +
                            " LEFT JOIN validation_table v ON DATE(v.timestamp) = d.day ";
                if (_loggedRoleName != "CG Superadmin")
                {
                    query2 = " AND organisation = '" + _loggedOrgName + "'";
                }
                //query = query + query2 + " GROUP BY DATE(timestamp) ORDER BY day";
                query = query + query2 + "GROUP BY d.day ORDER BY d.day";
    ...
    public int[] GetTestUserCounts()
    {
        ...
        try
        {
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                var query = @" SELECT 
                                SUM(CASE WHEN StartTime >= " + startdashboardfromdate +
                                      "  AND StartTime < CURRENT_DATE() + INTERVAL 1 DAY THEN 1 ELSE 0 END) AS monthCount, " +
                               "SUM(CASE WHEN DATE(StartTime) = CURRENT_DATE() THEN 1 ELSE 0 END) AS todayCount " +
                            " FROM cgstagingdata ";

                if (_loggedRoleName != "CG Superadmin")
                {
                    query2 = "  WHERE organization = '" + _loggedOrgName + "'";
                }
                query = query + query2;
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                var query = @" SELECT 
                            SUM(CASE 
                                ...
                                " AND StartTime < CURRENT_DATE() + INTERVAL 1 DAY THEN 1 ELSE 0 END) AS monthCount, " +
                            "SUM(CASE WHEN DATE(StartTime) = CURRENT_DATE() THEN 1 ELSE 0 END) AS todayCount " +
                            "FROM cgstagingdata WHERE reporturl IS NOT NULL AND reporturl != '' ";
    ...
    public int[] GetNewUserCounts()
    {
        ...
        try
        {
            ...
            using (var connection = new MySqlConnection(_connectionString))
            {
                ...
                var query = @"SELECT 
                            SUM(CASE 
		                        WHEN timestamp >= " + startdashboardfromdate +
                                " AND timestamp < CURRENT_DATE() + INTERVAL 1 DAY THEN 1 ELSE 0 END) AS monthCount, " +
                            " SUM(CASE WHEN DATE(timestamp) = CURRENT_DATE() THEN 1 ELSE 0 END) AS todayCount " +
                            " FROM validation_table WHERE register_page = 1 ";
                if (_loggedRoleName != "CG Superadmin")
                {
                    query2 = " AND organisation = '" + _loggedOrgName + "'";
                }
