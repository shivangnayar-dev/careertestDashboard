...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (element) {
        productListTable.add({
            ...
            product_name: `<a href="/ecommerce/productoverview" class="flex items-center gap-2">
                            <img src="`+ element.img + `" alt="Product images" class="h-6">
                            <h6 class="product_name">`+ element.product_name + `</h6>
                        </a>`,
            category: `<span class="category px-2.5 py-0.5 text-xs inline-block font-medium rounded border bg-slate-100 border-slate-200 text-slate-500 dark:bg-zink-800 dark:border-zink-700 dark:text-zink-400">` + element.category + `</span>`,
            price: element.price,
            stock: element.stock,
            revenue: element.revenue,
            status: `<span class="` + isStatus(element.status) + `">` + element.status + `</span>`,
            action: `<div class="relative dropdown">
                    <button class="flex items-center justify-center w-[30px] h-[30px] dropdown-toggle p-0 text-slate-500 btn bg-slate-100 hover:text-white hover:bg-slate-600 focus:text-white focus:bg-slate-600 active:text-white active:bg-slate-600 dark:bg-zink-800 dark:text-zink-400 dark:hover:bg-zink-700 dark:hover:text-white dark:focus:bg-zink-700 dark:focus:text-white dark:active:bg-zink-700 dark:active:text-white" id="productAction1" data-bs-toggle="dropdown"><i data-lucide="more-horizontal" class="w-3 h-3"></i></button>
                    <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-800" aria-labelledby="productAction1">
                        <li>
                            <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-700 dark:hover:text-zink-200 dark:focus:bg-zink-700 dark:focus:text-zink-200" href="/ecommerce/productoverview"><i data-lucide="eye" class="inline-block w-3 h-3 mr-1"></i> <span class="align-middle">Overview</span></a>
                        </li>
                        <li>
                            <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-700 dark:hover:text-zink-200 dark:focus:bg-zink-700 dark:focus:text-zink-200" href="/ecommerce/productcreate"><i data-lucide="file-edit" class="inline-block w-3 h-3 mr-1"></i> <span class="align-middle">Edit</span></a>
                        </li>
                        <li>
                            <a data-modal-target="deleteModal" class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-700 dark:hover:text-zink-200 dark:focus:bg-zink-700 dark:focus:text-zink-200" href="#!"><i data-lucide="trash-2" class="inline-block w-3 h-3 mr-1"></i> <span class="align-middle">Delete</span></a>
                        </li>
                    </ul>
                </div>`,
        });
