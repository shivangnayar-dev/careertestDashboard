    ...
    Array.from(datas).forEach(function (mailData, index) {
        ...
        <td class="px-3.5 py-2.5 border-y border-transparent first:pl-0 last:pr-0 w-20">
            <div class="flex items-center gap-3 ltr:pl-2 rtl:pr-2">
                <input type="checkbox" id="checkbox`+ mailData.id + `" value="` + mailData.id + `" class="itemCheckbox w-4 h-4 cursor-pointer bg-white border border-slate-200 checked:bg-none dark:bg-zink-900 dark:border-zink-700 rounded-sm appearance-none arrow-none relative after:absolute after:content-['\\eb7b'] after:top-0 after:left-0 after:font-remix after:leading-none after:opacity-0 checked:after:opacity-100 after:text-custom-500 checked:border-custom-500 dark:after:text-custom-500 dark:checked:border-custom-800" >
                <a href="#!" class="transition-all duration-200 ease-linear text-slate-500 hover:text-yellow-500 dark:text-zink-400 dark:hover:text-yellow-500 `+ checkStarred + `"><i data-lucide="star" class="w-4 h-4"></i></a>
                <a href="#!" class="transition-all duration-200 ease-linear text-slate-500 hover:text-yellow-500 dark:text-zink-400 dark:hover:text-yellow-500"><i data-lucide="step-forward" class="w-4 h-4"></i></a>
            </div>
        </td>
        <td class="relative px-3.5 py-2.5 border-y border-transparent first:pl-0 last:pr-0 text-slate-500 group-[.unread]/mail:text-slate-800 dark:text-zink-400 dark:group-[.unread]/mail:text-zink-50  mailBox">
        ...
        <td class="px-3.5 py-2.5 border-y border-transparent first:pl-0 last:pr-0 text-end w-20 text-slate-500 group-[.unread]/mail:text-slate-800 dark:text-zink-400 dark:group-[.unread]/mail:text-zink-50">`+ mailData.time + `</td>
    </tr>`
        lucide.createIcons();
        checkBoxAll();
        emailDetailShow()
